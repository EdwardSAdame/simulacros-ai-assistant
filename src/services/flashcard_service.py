...
from src.schemas.flashcard_schemas import FlashcardDeck
    ...
    def execute_flashcards_generation(
        ...
        if stream_manager:
            stream_manager.send_status_update(
                ...
                loading_phrases=["Estructurando conceptos", "Sintetizando respuestas", "Disenando cartas"]
            )
        ...
        try:
            ...
            if parsed_deck and hasattr(parsed_deck, 'flashcards'):
                cards_list = [card.dict() if hasattr(card, 'dict') else card for card in parsed_deck.flashcards]
